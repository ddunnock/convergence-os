export interface GlassCustomization {
  /**
   * Background color for the glass effect (e.g., "rgba(255, 255, 255, 0.1)" or
   * "#ffffff") Default: uses CSS variable --glass-bg
   */
  color?: string;

  /**
   * Transparency/opacity for the background (0-1) If provided, will override
   * the alpha channel in color
   */
  transparency?: number;

  /** Blur amount in pixels Default: uses CSS variable --blur (20px) */
  blur?: number | string;
  ...
  /** Border/outline width in pixels Default: 1px */
  outlineWidth?: number | string;

  /** Shadow for the glass effect Default: uses CSS variable --glass-shadow */
  shadow?: string;

  /**
   * Inner glow color and intensity (e.g., "rgba(255, 255, 255, 0.2)") Creates
   * an inset shadow for a glowing effect inside the element
   */
  innerGlow?: string;

  /** Inner glow blur/spread radius in pixels Default: 20px */
...
/**
 * Generates CSS custom properties for glass customization Useful for components
 * that need to pass styles to child elements
 */
